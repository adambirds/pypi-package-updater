    ...
    def test_write_error_handling(self):
        ...
        with patch('builtins.open', side_effect=mock_open_side_effect):
            with patch('pypi_updater.parser.logger.error') as mock_error:
                ...
                mock_error.assert_called_with(f"Error writing to file {req_file}: Permission denied")
...
class TestParserComplexScenarios:
    ...
    def test_three_way_circular_dependency(self):
        ...
        with patch('pypi_updater.parser.logger.warning') as mock_warning:
            ...
            assert any("Circular dependency detected involving" in str(call) for call in warning_calls)
    ...
    def test_comment_preservation_with_complex_comment(self):
        ...
        req_file = self.requirements_dir / "test.in"
        req_file.write_text("requests>=2.25.0  # Required for HTTP requests, see: https://docs.python-requests.org\n")
...
""")
        ...
        assert "flask>=2.0.0  #  Web framework" in content  # Note: >= preserved and extra space in comment
