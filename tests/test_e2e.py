...
""")
    ...
    def test_cli_check_only_mode(self, sample_requirements_structure):
        ...
        result = subprocess.run([
            "./.venv/bin/python", str(Path.cwd() / "update_packages.py"),
            "--check-only",
            "--requirements-dir", str(sample_requirements_structure / "requirements"),
...
""")
    ...
    def test_error_handling_invalid_directory(self):
        ...
        result = subprocess.run([
            "./.venv/bin/python", str(Path.cwd() / "update_packages.py"),
            "--check-only",
            "--requirements-dir", "/this/directory/does/not/exist"
        ], capture_output=True, text=True, cwd=Path.cwd())
