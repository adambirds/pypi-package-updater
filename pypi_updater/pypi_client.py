...
class PyPIClient:
            ...
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        logger.error(f"Failed to fetch info for '{package_name}': HTTP {response.status}")
                        return None
    ...
    async def _get_package_info_with_current(self, package_name: str, current_version: str) -> Optional[PackageInfo]:
